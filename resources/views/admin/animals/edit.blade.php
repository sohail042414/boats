...
<section class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1>Update Animal/Bird</h1>
      </div>
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="/">Home</a></li>
          <li class="breadcrumb-item"><a href="{{ url('/animals') }}">Animals/Birds</a></li>
...
<form autocomplete="off" role="form" style="width:100%" id="animal-form" method="POST" action="{{ url('/animals/'.$animal->id)}}" enctype="multipart/form-data">  
  <div class="row">
        ...
        <div class="card card-primary">
          ...
          <div class="card-body">
            <div class="form-group">
              ...
              <input type="text" name="name" id="name" placeholder="Animal/Bird Name" class="form-control" value="{{ old('name',$animal->name) }}">
              @if($errors->has('name'))
            ...
            <div class="form-group">
                <label for="electricity">Type</label>
                <select name="type" id="type" class="form-control custom-select">
                    <option selected disabled>Select one</option>  
                    @foreach ($animal_types as $key => $item)
                        @if($key == old('type',$animal->type))
            ...
            <div class="form-group">
              ...
              <input type="text" name="short_description" id="short_description" placeholder="Short Description" class="form-control" value="{{ old('short_description',$animal->short_description) }}">
...
@section('scripts')
    ...
    $(function() {
        $('#animal-images-table').DataTable({
            processing: true,            
            serverSide: true,
            ajax: '{!! route("animal-images-grid",["animal_id"=>$animal->id]) !!}',
            columns: [
                { data: 'id', name: 'id' },
                { data: 'name', name: 'name' },
                { data: 'id', name: 'id' },
    ...
    function deleteTableRow(row_id){
        ...
        $.ajax({
            ...
            success: function(response) {
              ...
              $('#animal-images-table').DataTable().rows().invalidate('data').draw(false);
            }
        });
    }
</script>
